...
impl GameDownload {
    ...
    pub fn advertise(&self) {
        ...
        self.app_handle.emit(&self.event_name, self.get_state()).expect("Failed to broadcast the download progress");
    }
    ...
    pub fn get_formated_remaining_time(&self) -> String {
        ...
        if seconds > 3600.0 {
            format!("{}h {}", (seconds / 3600.0).round() as u64, ((seconds % 3600.0) / 60.0).round() as u64)
